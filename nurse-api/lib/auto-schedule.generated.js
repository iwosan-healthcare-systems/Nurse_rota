...
exports.isPorterType = isPorterType;
exports.isPorterDayType = isPorterDayType;
...
function isFixedWeekdayWard(ward) {
...
function normalizeForCycleMatch(s) {
    if (s === "NC")
        return "N";
    if (s === "MWC")
        return "M";
...
function scheduleSingleNightSlotWard(group, days, startDate, leave, wardName, out, periodOffset, prev) {
    ...
    function nurseEffectiveBase(i) {
        return (phaseOverrides.get(sorted[i].id) ??
...
function scheduleCoverageNurses(group, days, startDate, leave, out, periodOffset = 0, prev = []) {
    ...
    function predictedShift(i, d) {
        ...
        if (relevantNcStart !== undefined)
            return NURSE_CYCLE[(d - (relevantNcStart + 8)) % CL];
    ...
    if (eligibleForNc.length > 0) {
        ...
        for (let b = 0; b < ncBlockCount; b++) {
            ...
            if (chosen === -1)
                continue; // every eligible nurse is on leave this block
    ...
    for (const [nurseIdx, starts] of [...ncStartDays.entries()]) {
        const validStarts = starts.filter((start) => {
            for (let k = start - 4; k < start; k++) {
                if (k < 0)
                    continue; // lead-in falls before this period — nothing to conflict with
                const kDate = new Date(startDate);
                kDate.setDate(kDate.getDate() + k);
                const kStr = ymd(kDate);
                if (mwcForcedM.get(kStr)?.has(nurseIdx))
                    return false;
                if (mwcByDate.get(kStr) === nurseIdx)
                    return false;
            }
            return true;
        });
        if (validStarts.length === starts.length)
            continue;
        if (validStarts.length > 0)
            ncStartDays.set(nurseIdx, validStarts);
        else
            ncStartDays.delete(nurseIdx);
    }
...
function generateSchedule(opts) {
    ...
    for (const ward of wards) {
        ...
        scheduleGroup(supervisors, wardCycle, days, opts.startDate, leave, ward.name, out, periodOffset + supervisorSeed, prev, fixedWeekday);
        ...
        scheduleGroup(surgicalDay, DAY_ONLY_CYCLE, days, opts.startDate, leave, ward.name, out, periodOffset + surgicalDaySeed, prev, true);
        ...
        if (isSingleNightSlotWard(ward)) {
            scheduleSingleNightSlotWard(regulars, days, opts.startDate, leave, ward.name, out, periodOffset + regularSeed, prev);
        }
        else {
            scheduleGroup(regulars, wardCycle, days, opts.startDate, leave, ward.name, out, periodOffset + regularSeed, prev, fixedWeekday);
        }
        scheduleGroup(naRegular, naCycle, days, opts.startDate, leave, ward.name, out, periodOffset + naRegularSeed, prev, fixedWeekday);
        scheduleGroup(naDay, DAY_ONLY_CYCLE, days, opts.startDate, leave, ward.name, out, periodOffset + naDaySeed, prev, fixedWeekday);
