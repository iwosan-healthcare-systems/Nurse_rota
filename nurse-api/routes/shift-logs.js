...
router.get('/current', wrap(async (req, res) => {
  ...
  if (!nurse_id || !shift_date) return res.status(400).json({ error: 'nurse_id and shift_date required' });
...
router.post('/', wrap(async (req, res) => {
  ...
  const { rows } = await pool.query(
    ...
    [
      ...
      is_swap || false, swap_note || null,
      is_leave || false, leave_request_id || null
    ]
  );
    ...
    for (const row of rows) {
      const { rows: inserted } = await client.query(
        `INSERT INTO shift_logs
         ...
         RETURNING *`,
        [
          row.nurse_id, row.shift_date, row.shift_type || 'LEAVE',
          row.started_at, row.expected_end_at, row.period_start,
          row.is_leave || true, row.leave_request_id || null,
          row.hours_logged || null, row.ended_at || null
        ]
      );
