...
// Chief Matron's rota-approval step is retired (CNO approves and publishes) —
...
const APPROVERS: SystemRole[] = ["admin", "cno", "head_nurse"];
...
const UNIT_PERIOD_MESSAGES: Record<string, (u: NonNullable<UnitPeriod>) => string> = {
  ...
  rota_autosubmitted_: (u) =>
    `${u.unitDisplay} · ${u.facilityDisplay} was auto-submitted for CNO review (T-17 deadline).`,
  ...
  rota_new_staff_: (u) =>
    `New staff added to ${u.unitDisplay} · ${u.facilityDisplay} mid-cycle — no shifts scheduled yet. Head nurse: generate and submit their schedule from the Rota page.`,
};
...
function RotaReminderBell({
  ...
  const workflowItem = (() => {
    ...
    if (isCnoOrAdmin && nextRotaStage === "submitted") {
